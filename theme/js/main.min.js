(() => {
  ...
  var MenuMobile = class {
    constructor(selector) {
      ...
      this.prevButtons.forEach((btn) => {
        btn.addEventListener("click", (event2) => {
          this.handlePrevButtonClick(event2);
        });
      });
      this.currentPanels = [];
      this._initAnchors();
    ...
    _initAnchors() {
      ...
      this.buttons.forEach((link) => {
        link.addEventListener("click", (event2) => {
          event2.preventDefault();
          const target = event2.target.getAttribute("href");
          if (target) {
    ...
    handlePrevButtonClick(event2) {
      event2.preventDefault();
      this.closePanel(this.currentPanels.at(-1));
    }
  };
  var menu_mobile_default = MenuMobile;

  // src/js/menu-drawer.js
  var Drawer = class {
    ...
    addBackdrop() {
      if (!this.backdrop) {
        ...
        this.backdrop.className = "fixed inset-0 z-30 bg-white/80 backdrop-blur-[20px]";
  ...
  var Select = class {
    ...
    selectOption(event2) {
      ...
      const selectedOptionText = event2.currentTarget.textContent;
      event2.currentTarget.classList.toggle("selected");
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    const isSigned = params.get("signed");
    if (isSigned) {
    ...
    const menuMobile = new menu_mobile_default("#nav-mobile");
    ...
    if (swipers) {
      swipers.forEach((el) => {
        const swiper4 = el.querySelector(".swiper");
    ...
    window.addEventListener("click", (event2) => {
      if (!dropdownButton.contains(event2.target) && !dropdownMenu.contains(event2.target)) {
        if (isDropdownOpen) {
          requestAnimationFrame(() => {
            dropdownMenu.classList.add("hidden");
            dropdownButton.setAttribute("aria-expanded", "false");
